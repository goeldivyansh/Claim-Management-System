...
class SubmitClaimServiceTest {
	...
	void testSubmitClaimMethodWithValidClaimObject() {
		...
		HospitalDTO providerDTO=new HospitalDTO(hospitalList);
		BenefitsDTO benefitsDTO=new BenefitsDTO(benefitList);
		ClaimAmountDTO claimAmountDTO=new ClaimAmountDTO(120000);
		
		when(policyClient.getProviders("P101", token)).thenReturn(new ResponseEntity<HospitalDTO>(providerDTO,HttpStatus.OK));
	...
	void testSubmitClaimWithInvalidPolicyId() {
		...
		String token="AAA";
		when(policyClient.getProviders("P1222", token)).thenThrow(PolicyException.class);
	...
	void testSubmitClaimWithNullProvider() {
		...
		HospitalDTO providerDTO=new HospitalDTO();
		BenefitsDTO benefitsDTO=new BenefitsDTO(benefitList);
		ClaimAmountDTO claimAmountDTO=new ClaimAmountDTO(120000);
